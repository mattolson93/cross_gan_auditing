...
class Trainer:
    ...
    def train(self) -> None:
        ...
        while iteration < self.iterations:
            ...
            if iteration % 3 == 0:
                ...
                self.visualizer3.visualize(trainepoch=str(self.epoch))
    ...
    def _train_loop(self, epoch: int, iterations: int) -> None:
        ...
        cur_gen_a, cur_model_a = self.generator , self.model
        cur_gen_b, cur_model_b = self.generator2, self.model2
        cur_gen_unused, cur_model_unused = self.generator3, self.model3
        #import pdb; pdb.set_trace()
        for i in range(iterations):
            temp = (cur_gen_a, cur_model_a)
            cur_gen_a, cur_model_a = (cur_gen_b, cur_model_b)
            cur_gen_b, cur_model_b = (cur_gen_unused, cur_model_unused)
            cur_gen_unused, cur_model_unused = temp

            # To device
            #z = self.wrapped_generator("sample_latent",self.batch_size)
            with torch.no_grad():
                z1_orig = cur_gen_a.sample_latent(self.batch_size).to(self.device)
                z2_orig = cur_gen_b.sample_latent(self.batch_size).to(self.device)

            # Apply Directions
            self.optimizer.zero_grad()
            #import pdb; pdb.set_trace()
            d1_grad, d1_nograd = cur_model_a(z1_orig)
            d2_grad, d2_nograd = cur_model_b(z2_orig,cur_model_a.selected_k, cur_model_a.unselected_k)


            # Original features
            with torch.no_grad():
                gen_feats1_combo_nograd = cur_gen_a.get_features( cat(z1_orig, d1_nograd))
                if self.exp("selfclr_lamb") > 0: selfclr_feats_orig, _ = torch.split(cur_gen_a.get_intermediate(), [z1_orig.shape[0],d1_nograd.shape[0]])

                gen_feats2_combo_nograd = cur_gen_b.get_features(cat(z2_orig, d2_nograd))
                ...
                #big gan hack
                #orig_feats2_combo = cur_gen_b.get_features(cat(z2_orig, z2_nograd))
                ...
                diffed_features1_nograd = self.diff_norm_feats(feats1_nograd, orig_feats1, cur_model_a.k - cur_model_a.batch_k)
                diffed_features2_nograd = self.diff_norm_feats(feats1_nograd, orig_feats2, cur_model_a.k - cur_model_a.batch_k)

            
            gen_feats1 = cur_gen_a.get_features(d1_grad)
            gen_feats2 = cur_gen_b.get_features(d2_grad)
            if self.exp("do_trans") > 0: gen_feats2 = self.trans(gen_feats2)

            # Forward
            feats1, feats2 = self.forward_projector((gen_feats1, gen_feats2))
            diffed_features1 = self.diff_norm_feats(feats1, orig_feats1, cur_model_a.batch_k)
            diffed_features2 = self.diff_norm_feats(feats2, orig_feats2, cur_model_a.batch_k)

            
            # Loss
            #the first two elements of features1 (for example) are from the SAME direction, just different starting zs
            label_order= torch.cat([cur_model_a.selected_k,cur_model_a.unselected_k])
            ...
            if self.dre_lamb > 0:
                #import pdb; pdb.set_trace()
                loss_dre = self.dre_lamb * self.dre_loss_fn(feats1, feats2, cur_model_a.selected_k< self.overlap_k, self.batch_size)
                out_string += f" ldre: {loss_dre.item():.3f}"
                loss += loss_dre 

            if self.dre_lamb < 0:
                loss_bce = -self.dre_lamb * self.bce_loss_fn(feats1, feats2, cur_model_a.selected_k< self.overlap_k, self.batch_size)
                out_string += f" lbce: {loss_bce.item():.3f}"
                loss += loss_bce 


            if self.exp("recon") > 0:
                color_loss1 = self.mse(orig_gen_feats1.repeat(cur_model_a.batch_k,1,1,1) , gen_feats1)
                color_loss2 = self.mse(orig_gen_feats2.repeat(cur_model_a.batch_k,1,1,1) , gen_feats2)
            ...
            if self.exp("selfclr_lamb") > 0:
                #import pdb; pdb.set_trace()
                selfclr_feat1 = cur_gen_a.get_intermediate()
                selfclr_feat1 = torch.reshape(selfclr_feat1 - selfclr_feats_orig.repeat(cur_model_a.batch_k,1,1,1),(selfclr_feat1.shape[0],-1))
            ...
            if self.exp("l2latent_lamb") > 0:
                #import pdb; pdb.set_trace()
                l2_latent_loss1 = self.mse(orig_feats1.repeat(cur_model_a.batch_k,1), feats1)
                l2_latent_loss2 = self.mse(orig_feats2.repeat(cur_model_a.batch_k,1), feats2)
            ...
            if self.grad_clip_max_norm is not None:
                torch.nn.utils.clip_grad_norm_(
                    cur_model_a.parameters(), self.grad_clip_max_norm
                )
