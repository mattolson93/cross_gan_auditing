...
def mymain(cfg: DictConfig) -> None:
    ...
    projector = AttClsModel("resnet18", 10, is_pretrained=False, dropout=cfg.dropout).to(device)
    ...
    generator.eval()
    generator2.eval()
    #generator.model.custom_out_resolution = 128
    #generator2.model.custom_out_resolution = 128
    ...
    for i in pbar:
        ...
        with torch.no_grad():
            ...
            ws2 = generator2.sample_latent(batch_size).to(device)
            img1 = generator(ws1)
            img2 = generator2(ws2)
        ...
        ws1_pred, _ = projector(img1)
        _, ws2_pred = projector(img2)

        #ws1_pred = ws1_pred[:batch_size]
        #ws2_pred = ws2_pred[batch_size:]

        img1_pred = generator(ws1_pred)
        img2_pred = generator2(ws2_pred)
        ...
        pixel_loss1 =  clip_wrapper(F.mse_loss(img1, img1_pred, reduction='none'), clip_val)
        pixel_loss2 =  clip_wrapper(F.mse_loss(img2, img2_pred, reduction='none'), clip_val)
        perc_loss1 =   clip_wrapper( percept(img1, img1_pred),   clip_val)
        perc_loss2 =   clip_wrapper( percept(img2, img2_pred),   clip_val)
        
        if i > (cfg.hparams.iterations - 50):
            with torch.no_grad():
                latent_loss1_avg.append(F.mse_loss(ws1,  ws1_pred).mean().item())
                latent_loss2_avg.append(F.mse_loss(ws2,  ws2_pred).mean().item())
                pixel_loss1_avg.append(F.mse_loss(img1, img1_pred).mean().item())
                pixel_loss2_avg.append(F.mse_loss(img2, img2_pred).mean().item())
                perc_loss1_avg.append(percept(img1, img1_pred).mean().item())
                perc_loss2_avg.append(percept(img2, img2_pred).mean().item())

        latent_loss = latent_loss1 + latent_loss2
        pixel_loss  = pixel_loss1  + pixel_loss2
        perc_loss   = perc_loss1 + perc_loss2


        loss =l2_lam *  latent_loss + mse_lam*pixel_loss + p_lamb* perc_loss
        
        if i % 10 == 0: 
            print(f' w_loss1: {latent_loss1.item():.4f}; pixel_loss1: {pixel_loss1.item():.4f} perc_loss1: {perc_loss1.item():.4f}')
            print(f' w_loss2: {latent_loss2.item():.4f}; pixel_loss2: {pixel_loss2.item():.4f} perc_loss2: {perc_loss2.item():.4f}')

        loss.backward()
        optimizer.step()
        pbar.set_description( f' w_loss: {latent_loss.item():.4f}; pixel_loss: {pixel_loss.item():.4f} perc_loss: {perc_loss.item():.4f}')
        

    latent_loss1_avg = np.array(latent_loss1_avg).mean()
    latent_loss2_avg = np.array(latent_loss2_avg).mean()
    ...
    latent_loss = (latent_loss1_avg + latent_loss2_avg)/2
    ...
    with open('results.csv', 'w') as f:
        f.write(f'summed, latent_loss,pixel_loss, perc_loss\n')
        f.write(f'{sumed},{latent_loss}, {pixel_loss}, {perc_loss}')
    with open('full_losses.csv', 'w') as f:
        f.write(f'latent_loss1,latent_loss2,pixel_loss1,pixel_loss2, perc_loss1,perc_loss2\n')
        f.write(f'{latent_loss1},{latent_loss2}, {pixel_loss1},{pixel_loss2}, {perc_loss1}, {perc_loss2}')
